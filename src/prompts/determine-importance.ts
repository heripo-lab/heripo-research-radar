...
function scoreScale(minimumScore: number): string {
  ...
  if (minimumScore === 1) {
    tiers.push(
      '1: **현재 실무 가치가 없는 정보** — 종료된 지원사업, 지난 행사, 만료된 입찰·채용 공고, 회비 납부 현황·회의록·내부 일정 같은 단순 행정 공지, 그리고 위 채용 판별에서 걸러진 비유산 분야 채용',
    );
  }
...
- 예외: 학술 성과(발간된 학술지, 공개된 연구, 종료된 학술대회 자료)는 참조 가치로 평가하며 이 규칙으로 깎지 않는다.
...
${temporalRule(minimumScore)}`;
    ...
    user: (context) => {
      ...
      const temporalCheck =
        minimumScore > 1
          ? ''
          : '\n\n점수를 매기기 전에, 기사에 적힌 마감일·접수 기간·행사일·입찰 마감·채용 기간이 위 발행일 기준으로 이미 지났는지 먼저 확인하라. 지났고 HARD RULE이 적용되면 1점이다.';
