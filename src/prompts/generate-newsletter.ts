...
3. **맺음말**: 오늘 다룬 핵심을 짧게 정리하고, 임박한 마감이나 진행 중인 일정을 짚어줍니다.
...
export const generateNewsletterPrompt: PromptBuilder<GenerateNewsletterPromptContext> =
  {
    system: (context) => {
      ...
      const subscribeRule = subscribePageUrl
        ? `\n\n## 구독 안내\n\n맺음말 **뒤에**, 본문 맨 마지막 줄로 구독 링크를 한 번만 넣는다. 브리핑이나 본문 중간에는 넣지 않는다.\n형식: \`[${newsletterBrandName} 구독하기](${subscribePageUrl})\` 앞에 권유 한 문장을 붙인다.`
        : '';
...
본문은 '습니다'체로 쓰고, 구독 링크는 맺음말 뒤 맨 마지막 줄에 한 번만 넣는다.`;
    },
  };
